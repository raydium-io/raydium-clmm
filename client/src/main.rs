...
fn load_cfg(client_config: &String) -> Result<ClientConfig> {
    ...
    let log_monitor = if config.getbool("Global", "log_monitor").unwrap() == None {
        false
    } else {
        config.getbool("Global", "log_monitor").unwrap().unwrap()
    };
...
fn main() -> Result<()> {
    ...
    if pool_config.log_monitor {
        loop {
            check_pool_fee_and_reward(&rpc_client, &pool_config, Pubkey::default()).unwrap();
            thread::sleep(time::Duration::from_secs(12 * 60 * 60));
        }
    } else {
        loop {
            ...
            match &v[0][..] {
                ...
                "create_pool" | "cpool" => {
                    if v.len() == 5 {
                        ...
                        let mint0_account =
                            spl_token::state::Mint::unpack(&rsps[0].as_ref().unwrap().data)
                                .unwrap();
                        let mint1_account =
                            spl_token::state::Mint::unpack(&rsps[1].as_ref().unwrap().data)
                                .unwrap();
                ...
                "p_all_protocol_position_by_pool" => {
                    ...
                    for position in position_accounts_by_pool {
                        ...
                        if protocol_position.pool_id == pool_config.pool_id_account.unwrap() {
                            println!(
                                "key:{}, protocol_position:{:?}",
                                position.0, protocol_position
                            );
                        }
                    }
                }
                ...
                "check_fee_reward_by_pool" => {
                    if v.len() == 2 || v.len() == 1 {
                        ...
                        check_pool_fee_and_reward(&rpc_client, &pool_config, filter_pool_id)
                            .unwrap();
                    }
                }
                "modify_pool" => {
                    ...
                    match param {
                        ...
                        Some(5) => {
                            ...
                            remaing_accounts
                                .push(AccountMeta::new(personal_position_address, false));
                            remaing_accounts
                                .push(AccountMeta::new(protocol_position_address, false));
                            val.push(v[5].parse::<u128>().unwrap());
                            val.push(v[6].parse::<u128>().unwrap());
                            val.push(v[7].parse::<u128>().unwrap());
                            println!("val:{:?}", val);
                        }
                        Some(6) => {
                            ...
                            match rsp {
                                ...
                                Some(rsp) => {
                                    let position = deserialize_anchor_account::<
                                        raydium_amm_v3::states::PersonalPositionState,
                                    >(&rsp)?;
                                    let (personal_position_key, __bump) =
                                        Pubkey::find_program_address(
                                            &[
                                                raydium_amm_v3::states::POSITION_SEED.as_bytes(),
                ...
                "decrease_liquidity" => {
                    if v.len() == 7 {
                        ...
                        if find_position.nft_mint != Pubkey::default()
                            && find_position.pool_id == pool_config.pool_id_account.unwrap()
                        {
                            let mut reward_vault_with_user_vault: Vec<(Pubkey, Pubkey)> =
                                Vec::new();
                ...
                "f_price_to_tick" => {
                    if v.len() == 5 {
                        ...
                        println!(
                            "f_price_to_tick price mint_decimals_0 mint_decimals_1 tick_spacing"
                        )
                    }
                }
                "tick_test" => {
                    ...
                    if v.len() == 2 {
                        ...
                        match sighash {
                            ...
                            [46, 156, 243, 118, 13, 205, 251, 178] => {
                                let ix =
                                    raydium_amm_v3::instruction::IncreaseLiquidity::deserialize(
                                        &mut &ix_data[..],
                                    )
                                    .map_err(|_| {
                                        anchor_lang::error::ErrorCode::InstructionDidNotDeserialize
                            ...
                            [160, 38, 208, 111, 104, 91, 44, 1] => {
                                let ix =
                                    raydium_amm_v3::instruction::DecreaseLiquidity::deserialize(
                                        &mut &ix_data[..],
                                    )
                                    .map_err(|_| {
                                        anchor_lang::error::ErrorCode::InstructionDidNotDeserialize
                            ...
                            [248, 198, 158, 145, 225, 117, 135, 200] => {
                                let ix = raydium_amm_v3::instruction::Swap::deserialize(
                                    &mut &ix_data[..],
                                )
                                .map_err(|_| {
                                    anchor_lang::error::ErrorCode::InstructionDidNotDeserialize
                            ...
                            [95, 135, 192, 196, 242, 129, 230, 68] => {
                                let ix =
                                    raydium_amm_v3::instruction::InitializeReward::deserialize(
                                        &mut &ix_data[..],
                                    )
                                    .map_err(|_| {
                                        anchor_lang::error::ErrorCode::InstructionDidNotDeserialize
                            ...
                            [141, 11, 197, 159, 168, 186, 252, 243] => {
                                ...
                                .unwrap();
                                let raydium_amm_v3::instruction::ModifyPool { param, val, index } =
                                    ix;
                                println!("param:{}, val:{:?}, index:{}", param, val, index);
                            }
                            _ => {
...
pub fn check_pool_fee_and_reward(
    rpc_client: &RpcClient,
    pool_config: &ClientConfig,
    ...
    let mut personal_infos: HashMap<Pubkey, Vec<(Pubkey, PersonalPositionState)>> = HashMap::new();
    ...
    let mut tick_array_infos: HashMap<Pubkey, Vec<(Pubkey, TickArrayState)>> = HashMap::new();
    ...
    for item in ret.into_iter() {
        ...
        match data_len {
            PoolState::LEN => {
                let pool =
                    deserialize_anchor_account::<raydium_amm_v3::states::PoolState>(&item.1)?;
            ...
            PersonalPositionState::LEN => {
                ...
                if personal_infos.contains_key(&personal.pool_id) {
                    ...
                    personal_infos.insert(personal.pool_id, personal_vec_one_pool.clone());
                }
            }
            TickArrayState::LEN => {
                let tick_array =
                    deserialize_anchor_account::<raydium_amm_v3::states::TickArrayState>(&item.1)?;
                ...
                if tick_array_infos.contains_key(&tick_array.pool_id) {
                    ...
                    tick_array_infos.insert(tick_array.pool_id, tick_array_one_pool.clone());
                }
            }
            _ => {
                continue;
    ...
    let vaults_iter = vault_tokens.chunks(100);
    let mut rsps: Vec<Option<solana_sdk::account::Account>> = Vec::new();
    for vaults in vaults_iter {
        let rsp = rpc_client.get_multiple_accounts(&vaults)?;
        rsps.extend(rsp);
    }
    for i in 0..vault_tokens.len() {
        let vault_key = vault_tokens[i];
        let vault_info =
            spl_token::state::Account::unpack(&rsps[i].as_ref().unwrap().data).unwrap();
    ...
    for (pool_id, personal_infos) in personal_infos.into_iter() {
        ...
        let vault0_info = token_infos.get(&pool_info.token_vault_0).unwrap().clone();
        let vault1_info = token_infos.get(&pool_info.token_vault_1).unwrap().clone();
        let reward_vault_info = if pool_info.reward_infos[0].token_vault == Pubkey::default() {
        ...
        for (personal_key, personal_info) in personal_infos.into_iter() {
            ...
            all_user_owed_reward_before += personal_info.reward_infos[0].reward_amount_owed;
            ...
            for array in tick_arrays.into_iter() {
                ...
                if array.0 == tick_lower_array_key {
                    ...
                    tick_lower_state = *tick_array
                        .get_tick_state_mut(tick_lower_index, pool_info.tick_spacing.into())
                        .unwrap();
                }
                if array.0 == tick_upper_array_key {
                    ...
                    tick_upper_state = *tick_array
                        .get_tick_state_mut(tick_upper_index, pool_info.tick_spacing.into())
                        .unwrap();
                }
            }
            if tick_lower_state.tick != tick_lower_index && tick_lower_state.tick != 0 {
                debug!(
            ...
            if tick_upper_state.tick != tick_upper_index && tick_lower_state.tick != 0 {
                debug!(
            ...
            personal_info.token_fees_owed_0 =
                raydium_amm_v3::instructions::increase_liquidity::calculate_latest_token_fees(
                    personal_info.token_fees_owed_0,
                    personal_info.fee_growth_inside_0_last_x64,
            ...
            if (personal_info.token_fees_owed_0 >= unclaimed_fee_0
                ...
                || (personal_info.reward_infos[0].reward_amount_owed >= unclaimed_reward
                    && personal_info.reward_infos[0].reward_amount_owed != 0)
            {
                warn!("fee_growth_inside_0_x64:{}, fee_growth_inside_1_x64:{}, reward_growths_inside:{}", fee_growth_inside_0_x64, fee_growth_inside_1_x64, reward_growths_inside[0]);
                warn!("@@@@@@@@@@@@@@@@@@@@ Too many fees or rewards @@@@@@@@@@@@@@@@@@@@");
            }
            ...
            all_user_owed_reward += personal_info.reward_infos[0].reward_amount_owed;
        }
        info!("all_user_liquidity:{}, owed_fee_0_before:{}, owed_fee_1_before:{}, owed_reward_before:{}, owed_fee_0:{}, owed_fee_1:{}, owed_reward:{}, owned_vault_0:{}, owned_vault_1:{}", all_user_liquidity, all_user_owed_fee_0_before, all_user_owed_fee_1_before, all_user_owed_reward_before, all_user_owed_fee_0, all_user_owed_fee_1, all_user_owed_reward, all_user_owned_vault_0, all_user_owned_vault_1);

        info!(
            "vault0:{}, vault1:{}, reward_vault:{}",
            pool_info.token_vault_0, pool_info.token_vault_1, pool_info.reward_infos[0].token_vault
        );
        info!(
        ...
        let need_claimed_0 = pool_info.total_fees_token_0 as i64 - (all_user_owed_fee_0) as i64;
        let need_claimed_1 = pool_info.total_fees_token_1 as i64 - (all_user_owed_fee_1) as i64;
        let need_claimed_reward = pool_info.reward_infos[0].reward_total_emissioned as i64
            - (all_user_owed_reward) as i64;
        info!(
        ...
        {
            warn!("*********************** pool liquidity, fee or reward not match with the actual situation ************************");
            warn!("poo_id:{}, pool_liquidity:{}, all_user_liquidity:{}, unclaimed_fee_0:{}, all_user_owed_fee_0:{}, unclaimed_fee_1:{}, all_user_owed_fee_1:{}, unclaimed_reward:{}, all_user_owed_reward:{}",
            pool_id, identity(pool_info.liquidity), all_user_liquidity, unclaimed_fee_0, all_user_owed_fee_0, unclaimed_fee_1, all_user_owed_fee_1, unclaimed_reward, all_user_owed_reward);
        }
    }
