...
fn fits_word(value: &U256) -> bool {
    let U256(ref arr) = *value; // Destructure to access the internal array.
    // Loop through the array from index 1 onwards and check if any element is non-zero.
    for i in 1..arr.len() {
        if arr[i] != 0 {
            return false;
        }
