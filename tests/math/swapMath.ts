  ...
  public static swapCompute(
    ...
    if (zeroForOne) {
      ...
      if (sqrtPriceLimitX64.gt(currentSqrtPriceX64)) {
        throw new Error("sqrtPriceX64 must smaller than currecy");
      }
    } else {
      ...
      if (sqrtPriceLimitX64.lte(currentSqrtPriceX64)) {
        throw new Error("sqrtPriceX64 must greater than currecy");
      }
    }
    ...
    while (
      ...
      const nextInitTick = tickDataProvider.nextInitializedTickWithinOneWord(
        state.tick,
        zeroForOne,
        tickSpacing
      );
      ...
      [state.sqrtPriceX64, step.amountIn, step.amountOut, step.feeAmount] =
        SwapMath.swapStepCompute(
          ...
          (
            zeroForOne
              ? step.sqrtPriceNextX64.lt(sqrtPriceLimitX64)
              : step.sqrtPriceNextX64.gt(sqrtPriceLimitX64)
          )
            ? sqrtPriceLimitX64
      ...
      if (state.sqrtPriceX64.eq(step.sqrtPriceNextX64)) {
        ...
        if (step.initialized) {
          const tickNext = tickDataProvider.getTick(step.tickNext);
